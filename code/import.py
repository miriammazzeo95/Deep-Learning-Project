...
model=KeyedVectors.load_word2vec_format('wiki-news-300d-1M.vec', limit=9999)
...
def vectorise(tweet):
    ...
    for word in words:
        if(model.__contains__(word)):
            vectorisedTweet.append(model.get_vector(word).astype(np.float32))
...
serie= itertuples_impl(df_politic)
targets_df = pd.DataFrame(data=serie)
targets_df.columns = ['targets']
ligne1_df = pd.DataFrame(data=targets_df.at[1,'targets'])
train_loader = data_utils.DataLoader(torch_tensor, batch_size = 2, shuffle = True)
...
mUSA_train = tfidf_USA[0:int(np.ceil(169033*persentage_train))]
mUSA_test = tfidf_USA[int(np.ceil(169033*persentage_train)):]
