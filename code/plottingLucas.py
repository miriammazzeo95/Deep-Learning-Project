...
def make_vae_plots(vae, x, y, outputs, training_data, validation_data,validation_data_politic,validation_data_UK, tmp_img="tmp_vae_out.png", figsize=(18, 18)):
    ...
    #     z = outputs['z']
    #     if z.shape[1] == 2:
    #         axes[0, 0].set_title(r'Latent Samples $\mathbf{z} \sim q_\phi(\mathbf{z} | \mathbf{x})$')
    #         qz = outputs['qz']
    #         plot_2d_latents(axes[0, 1], qz, z, y)
    #     else:
    #         axes[0, 0].set_title(r'Latent Samples $\mathbf{z} \sim q_\phi(\mathbf{z} | \mathbf{x})$ (t-SNE)')
    ...
    ax.plot(validation_data_politic['elbo'], label='politic')
    ax.plot(validation_data_UK['elbo'], label='UK')
    ax.legend()
    ...
    ax.plot(validation_data_politic['kl'], label='politic')
    ax.plot(validation_data_UK['kl'], label='UK')
    ax.legend()
    ...
    ax.plot(validation_data_politic['log_px'], label='politic')
    ax.plot(validation_data_UK['log_px'], label='UK')
    ax.legend()
